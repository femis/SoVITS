...
logger.info(f"追加项目根目录: {project_root}")
...
def get_tts_config(req):
    ...
    logger.info(f"本地请求的唯一Id: {unique_id}")
    # 检查缓存大小，如果超过20则清空缓存
    if len(tts_config_cache) > 20:
        logger.info("缓存大小超过10，清空缓存...")
    ...
    if unique_id not in tts_config_cache:
        logger.info(f"从文件加载配置并添加到缓存中: {tts_infer_yaml_path}")
...
def init_tts(req):
    ...
    logger.info(f"将TTS实例当前已存储： {len(tts_init_cache)}， 最大允许： {tts_init_max} 条")
...
# 下载文件
def download_file(url: str = '', max_retries: int = 3, delay: int = 5):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if response.status_code == 200:
                ...
                logger.info(f'文件已成功下载到系统缓存目录: {temp_file_path}')
        ...
        if attempt < max_retries - 1:
            logger.info(f'尝试 {attempt + 1} 失败，将在 {delay} 秒后重试...')
            time.sleep(delay)
    # 如果所有尝试都失败，则返回空字符串
    logger.info('所有重试均失败')
    return ""
...
def convert_mp3_to_wav(mp3_file_path, wav_file_path):
    ...
    logger.info('转WAV成功', wav_file_path)
...
def enter():
    ...
    is_makeing = 1
    logger.info(f"即将开始制作，当前时间：{get_time()}")
    ...
    end_time = time.time()
    logger.info(f"制作请求完成; 耗时：{end_time - start_time}秒，当前时间：{get_time()}")
...
def find():
    try:
        ...
        logger.error(f"处理 / 请求时发生异常: {e}")  # 记录异常信息
